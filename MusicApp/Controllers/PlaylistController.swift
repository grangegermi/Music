    ...
    func networkData () {
        ...
        ApiCaller.sharedCaller.getPlaylists(playlist: self.playlist) { [weak self] result in
            ...
            case .success(let model):
                ...
                if self!.playlistDetails.isEmpty {
                    var vc = UIAlertController(title: "Сообщение", message: "Плейлист недоступен ", preferredStyle: .alert)
        ...
        let action = UIAlertAction(title: "ОК", style: .default) { _ in
            ...
            DispatchQueue.main.async {
                ...
                vc.selectionHandler = { [weak self] playlist in
                    ApiCaller.sharedCaller.addTrackToPlaylist(track: (self?.playlistDetails[index].track)!, playlist: playlist) { succsses in
                        print("adding")
